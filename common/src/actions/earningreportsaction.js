      ...
      if (snapshot.val()) {
            ...
            if (renderobj[uniqueKey]) {
              if (status == "CANCELLED") {
                ...
                renderobj[uniqueKey].discountAmount = (
                  ...
                  parseFloat(mainArr[j].discount_amount)
                ).toFixed(settings.decimal);
                ...
                renderobj[uniqueKey].customerPaid = (
                  ...
                  parseFloat(mainArr[j].customer_paid)
                ).toFixed(settings.decimal);
              ...
              if (status == "CANCELLED") {
                ...
                renderobj[uniqueKey]["driverShare"] = (0).toFixed(
                  settings.decimal
                );
                renderobj[uniqueKey]["customerPaid"] = (0).toFixed(
                  settings.decimal
                );
                renderobj[uniqueKey]["convenienceFee"] = (0).toFixed(
                  settings.decimal
                );
                renderobj[uniqueKey]["tripCost"] = (0).toFixed(
                  settings.decimal
                );
                renderobj[uniqueKey]["cancellationFee"] = parseFloat(
                  mainArr[j].cancellationFee
        ...
        if (renderobj) {
          const arr = Object.keys(renderobj).map((i) => {
            renderobj[i].myEarning = (
              parseFloat(renderobj[i].customerPaid) +
              parseFloat(renderobj[i].cancellationFee) -
              parseFloat(renderobj[i].driverShare)
            ).toFixed(settings.decimal);
            renderobj[i].customerPaid = (
              parseFloat(renderobj[i].customerPaid) +
              parseFloat(renderobj[i].cancellationFee)
            ).toFixed(settings.decimal);
            renderobj[i].rideCost = (
              parseFloat(renderobj[i].tripCost) -
              parseFloat(renderobj[i].convenienceFee)
            ).toFixed(settings.decimal);
            renderobj[i].driverShare = parseFloat(
              renderobj[i].driverShare
            ).toFixed(settings.decimal);
            return renderobj[i];
          });
          dispatch({
