...
export const fetchUserNotifications = () => (dispatch) => (firebase) => {
  ...
  onValue(
    child(singleUserRef(auth.currentUser.uid), "notifications"),
    (snapshot) => {
...
export const editNotifications =
  (notification, method) => (dispatch) => (firebase) => {
    const { notifyRef, notifyEditRef } = firebase;
    dispatch({
...
export const sendNotification =
  (notification) => (dispatch) => async (firebase) => {
    dispatch({
      ...
      for (let i = 0; i < users.length; i++) {
        ...
        if (
          ...
          if (
            ...
            usr.userPlatform === notification.devicetype
          ) {
            obj.to = usr.pushToken;
            arr.push(obj);
          }
