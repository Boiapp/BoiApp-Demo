...
export const fetchPlacesAutocomplete = (searchKeyword) => (firebase) => {
  return new Promise((resolve, reject) => {
    ...
    fetch(`https://${config.project}.web.app/googleapis-autocomplete`, {
      method: "POST",
      headers: {
...
export const fetchCoordsfromPlace = (place_id) => (firebase) => {
  return new Promise((resolve, reject) => {
    const { config } = firebase;
    fetch(`https://${config.project}.web.app/googleapis-getcoords`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Basic " + base64.encode(username + ":" + password),
...
export const fetchAddressfromCoords = (latlng) => (firebase) => {
  return new Promise((resolve, reject) => {
    const { config } = firebase;
    fetch(`https://${config.project}.web.app/googleapis-getaddress`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Basic " + base64.encode(username + ":" + password),
...
export const getDirectionsApi =
  (startLoc, destLoc, waypoints) => (firebase) => {
    return new Promise((resolve, reject) => {
      const { config } = firebase;
      const body = {
        start: startLoc,
      ...
      fetch(`https://${config.project}.web.app/googleapis-getdirections`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: "Basic " + base64.encode(username + ":" + password),
