...
export default function AppCommon({ children }) {
  ...
  useEffect(() => {
    if (languagedata.langlist) {
      ...
      if (connector.connected) {
        ...
        // api.checkUserExists(walletId).then((res) => {
        //   if (res.user.uid) {
        //     api
        //       .walletSignIn(res.user.uid)
        //       .then((res) => {
        //         dispatch(api.fetchUser(res));
        //       })
        //       .catch((err) => {
        //         dispatch(api.fetchUser(null));
        //       });
        //   } else dispatch(api.fetchUser(null));
        // });
        dispatch(api.fetchUser(null));
      } else dispatch(api.fetchUser(null));
    }
  ...
  const saveToken = async () => {
    ...
    if (
      (auth.info.profile.pushToken && auth.info.profile.pushToken != token) ||
      !auth.info.profile.pushToken
    ) {
      dispatch(
        api.updatePushToken(
          auth.info,
  ...
  const StartForegroundGeolocation = async () => {
    ...
    // watcher.current = await Location.watchHeadingAsync((heading) => {
    //   store.dispatch({
    //     type: "UPDATE_GPS_HEADING",
    //     payload: {
  ...
  useEffect(() => {
    if (
      ...
      settings &&
      initialFunctionsNotCalled.current
    ) {
  ...
  if (
    authStillNotResponded.current ||
    !(languagedata && languagedata.langlist) ||
    !settings ||
    authState.current == "loading"
  ) {
