...
export default function PaymentDetails(props) {
  ...
  const doPayment = (payment_mode) => {
    if (payment_mode == "cash" || payment_mode == "wallet") {
      ...
      curBooking.discount_amount = parseFloat(payDetails.discount).toFixed(
        settings.decimal
      );
    ...
    <View style={styles.mainView}>
      ...
      >
        <View style={{ flex: 1, flexDirection: "column" }}>
          ...
          {useWalletCash ||
          (booking.status != "PAYMENT_PENDING" && booking.usedWalletMoney) ? (
            <View
              style={{
                flex: 1,
                flexDirection: isRTL ? "row-reverse" : "row",
            ...
            >
              ...
              {settings.swipe_symbol === false ? (
                ...
                >
                  ...
                  {payDetails
                    ? payDetails.usedWalletMoney
                      ? parseFloat(payDetails.usedWalletMoney).toFixed(
                          settings.decimal
                        )
                      : "0.00"
                    : "0.00"}{" "}
